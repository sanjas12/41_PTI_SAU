...
class IntervalControl(CollapsibleGroupBox):
    ...
    def update_plc_info(self, interval: float):
        ...
        if interval < 0.05:
            ...
            self.plc_status_label.setStyleSheet("color: #f44336;")
        elif interval < 0.1:
            self.plc_status_label.setText("⚡ Очень часто! Рекомендуется 100-500 мс")
            self.plc_status_label.setStyleSheet("color: #FF9800;")
        elif interval <= 0.5:
            self.plc_status_label.setText("✅ Оптимальный интервал")
            self.plc_status_label.setStyleSheet("color: #4CAF50;")
        else:
    ...
    def _update_status(self):
        ...
        plc_freq = (
            1.0 / self._current_plc_interval if self._current_plc_interval > 0 else 0
        )
