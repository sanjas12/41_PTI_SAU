...
class PLCInterface(QObject):
    ...
    def __init__(self, generator: SignalGenerator, parent=None):
        ...
        # Для REAL чисел используем по 2 регистра (32-bit float)
        self.registers = {
            # Аналоговые сигналы (каналы 1-20) - REAL
            'analog_signals': {
                'start_address': 0,  # %MW0 - %MW39 (20 каналов * 2 регистра)
                'count': 20,
                'type': 'float'
            },
            # Управляющие параметры
            'control': {
                'start_address': 40,  # %MW40
                'count': 10,
                'type': 'int'
            },
            # Статусная информация
            'status': {
                'start_address': 50,  # %MW50
                'count': 10,
                'type': 'int'
            }
        }
        ...
        self.write_interval = 0.1  # 100ms между записями
    ...
    def _write_all_data(self):
        ...
        with self._lock:
            ...
            # Пример: записываем статус каналов (включен/выключен)
            status_word = 0
            ...
            active_count = sum(1 for ch in self.generator.channels if ch.enabled)
            write_data.append((status_addr, active_count))
            
            # Собираем данные для пакетной записи
            # Группируем по адресам для минимизации запросов
            write_groups = {}
            for addr, value in write_data:
                if addr not in write_groups:
                    write_groups[addr] = []
                write_groups[addr].append(value)
                
            # Выполняем запись
            for addr, values_list in write_groups.items():
                if len(values_list) == 1:
                    self.modbus.write_single_register(addr, values_list[0])
                else:
                    self.modbus.write_multiple_registers(addr, values_list)
                    
    def update_plc_data(self):
        """Обновить данные в PLC (вызывается по таймеру)"""
        if not self.running or not self.modbus.is_connected():
            return
    ...
    def read_float(self, address: int) -> Optional[float]:
        ...
        try:
            ...
            if data and len(data) == 2:
                # Преобразуем 2 регистра в float
