...
class PLCInterface(QObject):
    ...
    def __init__(self, generator: SignalGenerator, parent=None, debug: bool = False):
        ...
        self.registers: Dict[str, Dict[str, Any]] = {
            "analog_signals": {"start_address": 0, "count": 20, "type": "float"},
            "control": {"start_address": 40, "count": 10, "type": "int"},
            "status": {"start_address": 50, "count": 10, "type": "int"},
        }

        # Кэш данных для записи
        self.write_cache: Dict[int, int] = {}
        ...
        self.last_written_data: Dict[str, Any] = {}
        self.write_count = 0
    ...
    def configure(self, host: str, port: int, unit_id: int = 1):
        ...
        try:
            ...
            if self.debug:
                print(
                    f"[PLC_DEBUG] Настроен Modbus: {host}:{port} (Unit ID: {unit_id})"
                )
    ...
    def _prepare_write_data(self):
        ...
        with self._lock:
            try:
                ...
                write_data = {"registers": [], "channels": [], "timestamp": time.time()}
                ...
                for i, value in enumerate(values):
                    ...
                    try:
                        ...
                        self.error_occurred.emit(
                            f"Ошибка подготовки канала {i + 1}: {e}"
                        )
                ...
                active_count = sum(
                    1 for ch in self.generator.channels[:20] if ch.enabled
                )
    ...
    def update_plc_data(self):
        ...
        task.signals.error.connect(
            lambda e: self.error_occurred.emit(f"Ошибка записи: {e}")
        )
    ...
    def set_write_interval(self, interval: float):
        ...
        if self.debug:
            print(
                f"[PLC_DEBUG] Интервал записи изменен: {self.write_interval:.3f} с "
